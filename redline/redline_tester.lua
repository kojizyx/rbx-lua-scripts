-- redline_tester.lua | koji_xyz | v1
-- test build. new stuff goes here first, gets pushed to main when its ready.

if _G.rlt_cleanup then pcall(_G.rlt_cleanup); task.wait(0.1) end

local rn = notify
rn("loading tester...","Redline",3)
...
local cfg_defaults = {
    ...
    theme='purple',
    auto_save=true,
    spd=false, spd_top=140, spd_rmp=50,
    mac=false, mac_key="x",
}
...
local phx_log={t0=0,dist=0,sched=0,press_t=0,active=false}
local mp_unk={}
local mp_skip={}; for _,v in ipairs({"GunImpact","BulletHole","BulletSmoke","BulletSpark","MuzzleFlash","GunHit","CastigateImpact","BulletDecal","ImpactParticle"}) do mp_skip[v]=true end
...
local function r_float(a) if not mem_on or not a or a==0 then return 0 end local ok,v=pcall(memory_read,"float",a); return (ok and tonumber(v)) or 0 end
local function r_rbxstr(a)
...
-- camera rotation offsets (for velocity speed direction)
local cam_od={}; local cam_ok=false; local cam_cPtr=0

local function load_cam_od()
    if not mem_on then return end
    ...
    if r.p1 and r.p2 and r.ws and r.cm and r.cr then cam_od=r; cam_ok=true end
end
pcall(load_cam_od)

local cam_BASE=type(getbase)=="function" and getbase() or 0

local function resolve_cam()
    cam_cPtr=0; if not cam_ok or not mem_on or cam_BASE==0 then return 0 end
    local a=r_ptr(cam_BASE+cam_od.p1); if a==0 then return 0 end
    local b=r_ptr(a+cam_od.p2);       if b==0 then return 0 end
    local c=r_ptr(b+cam_od.ws);       if c==0 then return 0 end
    cam_cPtr=r_ptr(c+cam_od.cm); return cam_cPtr
end

local function get_look()
    if cam_cPtr==0 and resolve_cam()==0 then return nil end
    local base=cam_cPtr+cam_od.cr
    local x,y,z=r_float(base+8),r_float(base+20),r_float(base+32)
...
local PQ_MIN_GAP=80
local last_gp_press=0
local last_parry_conf=0
...
local function calc_sched(gun, dist)
    ...
    if gun=="phoenix" then
        ...
        phx_flight=true
        if not phx_log.active then phx_log.t0=ms(); phx_log.active=true; phx_log.press_t=0 end
...
on_window=function(gun_guess,epos,src,certain)
    ...
    local gun=gun_guess
    if certain then st.last_gun=gun; st.gun_t=ms() end
    if att then
        if certain then
    ...
    if shot.claimed then
        if gun~=shot.gun and (certain or not shot.certain) then
            ...
            local fire_at=(shot.t0 or ms())+nsched
            ...
            elseif certain and fire_at>ms()+20 then
                shot.entry=enqueue_parry(fire_at-ms(),att,gun,epos)
    ...
    t_win=ms()
    win_last[who]=ms()+math.max(300,sched)+350
...
on_flash=function(gun,fpos)
    ...
    if gun=="phoenix" and phx_log.active and phx_log.t0>0 then
        ...
        local lead=(phx_log.press_t>0) and (ms()-phx_log.press_t) or -1
    ...
    if gun=="siege" and cfg.s2 and cfg.gp then
        local now_s2=ms()
        if now_s2-siege_s2_t>((cfg.s2_w2f or 1000)+300) then
            siege_s2_t=now_s2
            local ping=cfg.ping or 47
            local mg_tbl=(type(cfg.mg)=="table") and cfg.mg or {}
            local sched2=math.max(50,(cfg.s2_w2f or 1000)-(mg_tbl.siege or 200)-ping)
            task.spawn(function()
                task.wait(sched2/1000)
...
on_parry=function(ev)
    ...
    if ours then
        if ms()-last_parry_conf<350 then return end
        last_parry_conf=ms()
        st.parry_t=ms(); miss_n=0; pq_last_press=ms()
        gp_lock=ms()+250
        for _,e in ipairs(parry_queue) do
            if not e.done and e.fire_at<=ms()+260 then e.done=true end
        end
...
local function dump_attrs(obj,label)
    ...
    while node~=0 and guard<24 do
        guard=guard+1
        local row={}
        for o=0,0x78,0x8 do row[#row+1]=string.format("+%02x=0x%x",o,r_ptr(node+o)) end
        log("[attr] node"..guard.." @0x"..string.format("%x",node))
        log("[attr]   "..table.concat(row," "))
        local nm=r_rbxstr(node)
        local val=r_ptr(node+OFF.attr_val)
...
local function scan_pgui()
    ...
    for eff_nm,gun in pairs(pgui_map) do
        ...
        if not pg_seen[addr] then
            pg_seen[addr]=now; st.last_gun=gun; st.gun_t=ms()
        ...
        if elapsed>=delay then
            ...
            task.spawn(function()
                dlog("[gp] F -> ".._gun.." (pgui)")
                if not gun_press() then return end
                pg_last_press=ms()
            end)
        end
    end
...
local function update_esp()
    ...
    if tick()<warn_til then
        ...
        local alpha=base_a
        if cfg.warn_fade then alpha=base_a*frac end
...
local hb_orig=setmetatable({},{__mode="k"})
...
local function sl_pick()
    local tgts=tgts_cached(); if #tgts==0 then return nil end
...
local function do_sl()
    ...
    local ap=live_aim_pos(t); if not ap then st.sl_tgt=nil; return end
...
run.RenderStepped:Connect(function(dt)
    ...
    if esp_acc<0.016 then return end
    esp_acc=0
    update_esp()
end)
...
task.spawn(function()
    while loops_active do
        if cfg.auto_ping and type(GetPingValue)=="function" then
            ...
            if ok and tonumber(p) and p>0 then cfg.ping=cl(fl(p),0,2000) end
        end
        task.wait(2)
    end
end)
...
task.spawn(function()
    ...
    while loops_active do
        ...
        local rt=get_root()
        if not(cfg.spd and rt) then spd_vacc=nil; continue end
        ...
        if spd_vacc==nil then spd_vacc=Vector3.new(rv.X,0,rv.Z).Magnitude end
        local step=cfg.spd_rmp*dt
        spd_vacc=spd_vacc+cl(cfg.spd_top-spd_vacc,-step,step)
        rt.AssemblyLinearVelocity=Vector3.new(flat.X*spd_vacc,rv.Y,flat.Z*spd_vacc)
    end
end)

-- speed macro
local mac_busy=false
local function do_mac()
    if not cfg.mac or mac_busy then return end
    mac_busy=true
    keyrelease(keys.w)
    keypress(keys.s);         task.wait(0.02)
    keypress(keys.leftshift); task.wait(0.01)
    keypress(keys.spacebar);  mouse2press()
    keyrelease(keys.s);       task.wait(0.05)
    keyrelease(keys.leftshift); mouse2press()
    task.wait(0.05); task.wait(0.05)
    mouse2release(); keypress(keys.w); task.wait(0.1)
    mac_busy=false
end
...
local function mhex(n) return mac_key_tbl[n] or 0x58 end

task.spawn(function()
    while loops_active do
        if cfg.mac then
            local dn=false; pcall(function() dn=iskeypressed(mhex(cfg.mac_key)) end)
            if dn then task.spawn(do_mac) end
        end
        task.wait(0.1)
    end
end)

_G.rlt_cleanup=function()
    loops_active=false
    rm_hud(); rm_fov()
...
task.spawn(function()
    ...
    local Window=UiLib.CreateWindow({
        Title  = "Redline Tester  v1   |   koji_xyz",
        X      = 70,
        Y      = 50,
        Width  = 640,
        Height = 640,
    ...
    local mac_keys={"q","e","r","t","g","h","z","x","c","v","b","n"}
    local kbMenu, kbHb
    ...
    local function reset_cfg()
        ...
        cfg_save()
        pcall(function() UiLib.Notify("Redline Tester","config reset to default  (reinject to refresh menu)",4) end)
    end
    ...
    pcall(function()
        ...
        Window.AddToggle(c,"Enable",cfg.aura,function(s) cfg.aura=s; if not s then aura_pending=false; if cfg.aura_hb then restore_hurtboxes() end end; mark_chg() end)
    ...
    pcall(function()
        local c=Window.AddCategory("Movement")
        Window.AddSection(c,"Velocity Speed")
        Window.AddToggle(c,"Enable",cfg.spd,function(s) cfg.spd=s; if not s then spd_vacc=nil end; mark_chg() end)
        tip(Window.AddSlider(c,"Target Studs/s",16,250,cfg.spd_top,function(v) cfg.spd_top=fl(v); mark_chg() end),"top speed to ramp to in studs per second")
        tip(Window.AddSlider(c,"Ramp Rate",1,250,cfg.spd_rmp,function(v) cfg.spd_rmp=fl(v); mark_chg() end),"how fast it reaches target speed. lower = smoother acceleration")
        Window.AddSection(c,"Speed Macro")
        Window.AddToggle(c,"Enable",cfg.mac,function(s) cfg.mac=s; mark_chg() end)
        Window.AddDropdown(c,"Trigger Key",mac_keys,cfg.mac_key,function(sel) cfg.mac_key=sel; mark_chg() end)
        Window.AddValueLabel(c,"Sequence","S+shift+space+M2 -> W")
    end)
    ...
    pcall(function()
        ...
        Window.AddValueLabel(c,"Dev","koji_xyz")
        Window.AddValueLabel(c,"Build","tester v1")
    end)
            ...
            if active then
                ...
                if dn and not last then
                    if cfg.hb then
                        ...
                        pcall(function() UiLib.Notify("Redline Tester","hitbox "..(st.hb_on and "ON" or "OFF"),1) end)
                    end
                end
                last=dn
            end
    ...
    pcall(function() UiLib.Notify("Redline Tester","loaded  |  koji_xyz  (Ctrl = menu)",5) end)
    log("[rlt] v1 | MatchaUI | fps "..get_fps())
